...
const { fetchSuccess: fetchUserSuccess } = user.actions;
const { fetchSuccess: fetchTokenSuccess } = token.actions;

const removeToken = () => async (dispatch) => {
  window.localStorage.removeItem("token");
  await dispatch(fetchTokenSuccess({ token: null }));
};

const removeUser = () => async (dispatch) => {
  await dispatch(fetchUserSuccess(null));
};

export const logout = () => async (dispatch) => {
  await dispatch(removePhotos());
